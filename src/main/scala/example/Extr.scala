...
object Extr {
  ...
  case class Settings(fetchSize: Int, consistencyLevel: ConsistencyLevel, threads: Int)
    ...
    parseArguments(new Conf(args)).foreach { conf =>
      ...
      Try {
        conf.mode match {
          ...
          case "export2" =>
            Await.result(
              exporter2(session, conf.selection.keyspace, conf.selection.table, conf.settings.threads, conf.flags.showProgress),
              Inf)
        }
      } match {
        case Success(_) =>
        case Failure(e) =>
          Console.err.println(
            s"\nError during '${conf.mode}': message: '${if(e != null) e.getMessage else ""}'")
          System.exit(1)
      }
  ...
  def parseArguments(conf: Conf) =
    for {
      ...
      beQuiet <- conf.quiet.toOption
      threads <- conf.threads.toOption
